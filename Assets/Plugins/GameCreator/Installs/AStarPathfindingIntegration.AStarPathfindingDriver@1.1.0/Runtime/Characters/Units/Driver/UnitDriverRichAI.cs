...
{
	[Title("A* Rich AI Path Agent")]
	[Image(typeof(IconCharacterWalk), ColorTheme.Type.Yellow, typeof(OverlayArrowRight))]
	[Category("A* Rich AI Path Agent")]
	[Description(
		"Moves the Character using A* Pathfinding's Rich AI Path Agent. " +
		"Requires a scene with an A* Pathfinder and Navmesh"
	)]

	[Serializable]
	public class UnitDriverRichAI : TUnitDriver
	{
		// EXPOSED MEMBERS: -----------------------------------------------------------------------
		...
		[SerializeField] private AutoRepathPolicy m_AutoRepath = new AutoRepathPolicy();
		...
		// MEMBERS: -------------------------------------------------------------------------------
		[NonSerialized] protected RichAI Agent;
		...
		public override void OnStartup(Character character)
		{
			...
			this.Agent = this.Character.GetComponent<RichAI>();
			if (this.Agent == null)
			{
				this.Agent = this.Character.gameObject.AddComponent<RichAI>();
			}
			...
			this.Agent.autoRepath = this.m_AutoRepath;
			this.Agent.whenCloseToDestination = this.m_WhenCloseToDestination;
		}
		...
		protected virtual void UpdateProperties(IUnitMotion motion)
		{
			...
			this.Agent.autoRepath = m_AutoRepath;
			this.Agent.whenCloseToDestination = this.m_WhenCloseToDestination;
		}
		...
		protected virtual void UpdateTranslation(IUnitMotion motion)
		{
			...
			{
				Debug.LogWarning("RichAI component is not enabled", this.Character.gameObject);
				return;
			}
		...
		// STRING: --------------------------------------------------------------------------------
		public override string ToString() => "A* Rich AI Pathfinding";
	}
}
